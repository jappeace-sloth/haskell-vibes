...
runRuleReview session paths = do
  ...
  when (not disabled && stackReady) $ do
    -- Validate before claiming: configuration errors must leave the edits queued.
    model <- reviewModel RuleReviewer
    ...
    when claimedReady (reviewClaimed session paths model)

reviewClaimed :: Text -> TurnPaths -> Text -> IO ()
reviewClaimed session paths model = do
  ...
  -- Phase A Claude reviewer model. Decision: Sonnet rather than Haiku. The review only
