...
let
  ...
  env = pkgs.buildEnv {
    ...
    # `#!/usr/bin/env bash` shebang execute. The image only ships
    # /bin and /usr/local/bin otherwise, so the PostToolUse hook
    # (and any other env-shebanged script copied in later) would
